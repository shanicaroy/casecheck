...
trust_issue: set true on dimension H only, and only with a weak or missing verdict, when a stated outcome cannot be true (a launch curve for a concept that was never launched, a metric for an unshipped project) or cannot be verified (a round number with no source, testimonials that read as invented). A trust problem outranks every ordinary gap in the final ranking, so raise it only when the evidence is in the text, and quote it. For every other dimension, trust_issue is false.
